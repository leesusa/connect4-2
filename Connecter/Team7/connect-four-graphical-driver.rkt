...
(define preferences-menu-item
  (instantiate menu-item%
    ...
    (callback
     (lambda (menu-item event)
       ;(send width-text-field set-value (~a WIDTH))
       ;(send height-text-field set-value (~a HEIGHT))
       ;(send cell-size-text-field set-value (~a CELL-SIZE))
       ;(send player-1-exe-text-field set-value (~a exe-1))
       ;(send player-1-args-text-field set-value (~s args-1))
       ;(send player-2-exe-text-field set-value (~a exe-2))
       ;(send player-2-args-text-field set-value (~s args-2))
       (send dialog show #t)))))
